...
export default function AdminDashboard({ email, name }: { email: string; name: string }) {
  ...
  async function loadInquiries() {
    ...
    try {
      ...
      const data = await response.json().catch(() => null) as { inquiries?: Inquiry[]; error?: string } | null;
      if (!response.ok) throw new Error(data?.error || "Inquiries could not be loaded.");
      setInquiries(data?.inquiries ?? []);
    } catch (error) {
      setNotice(error instanceof Error ? error.message : "Inquiries could not be loaded. Refresh and try again.");
    } finally {
...
function InquiryList({ inquiries, loading, onRefresh }: { inquiries: Inquiry[]; loading: boolean; onRefresh: () => void }) { return <section className="cms-panel inquiry-panel"><div className="panel-title"><div><span className="cms-kicker">Contact form</span><h2>Collaboration inquiries</h2></div><div className="inquiry-tools"><span className="cms-muted">{inquiries.length} saved</span><button className="cms-ghost" onClick={onRefresh} disabled={loading}>{loading ? "Refreshing…" : "Refresh"}</button></div></div>{loading ? <p className="cms-empty">Loading inquiries…</p> : inquiries.length ? <div className="inquiry-list">{inquiries.map((inquiry) => <article className="inquiry-card" key={inquiry.id}><div className="inquiry-card-head"><div><h3>{inquiry.name}</h3><span>{inquiry.brand || "Independent inquiry"} · {inquiry.projectType || "Project details"}</span></div><a className="cms-ghost" href={`mailto:${inquiry.email}?subject=${encodeURIComponent(`Re: Aura Studio inquiry from ${inquiry.name}`)}`}>Reply by email ↗</a></div><p>{inquiry.message}</p><small>{inquiry.email} · {new Date(inquiry.createdAt).toLocaleString()}</small></article>)}</div> : <p className="cms-empty">No inquiries yet. New contact form messages will appear here.</p>}</section>; }
