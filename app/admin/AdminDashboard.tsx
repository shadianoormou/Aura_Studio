...
export default function AdminDashboard({ email, name }: { email: string; name: string }) {
  ...
  async function handleLibraryUpload(file: File) {
    ...
    try {
      ...
      setNotice("Uploaded. Click ‘Add to portfolio’ to choose its title, category and publishing status.");
...
function MediaLibrary({ assets, loading, onUpload, onCopy, onUse }: { assets: MediaAsset[]; loading: boolean; onUpload: (file: File) => void; onCopy: (url: string) => void; onUse: (asset: MediaAsset) => void }) {
  return <section className="cms-panel media-panel"><div><span className="cms-kicker">Optimised storage</span><h2>Media library</h2><p>Upload a cover image or video here. After upload, click <b>Add to portfolio</b> to choose its category and publish it.</p><label className="drop-zone"><input type="file" accept="image/jpeg,image/png,image/webp,image/avif,video/mp4,video/webm" onChange={(event) => event.target.files?.[0] && onUpload(event.target.files[0])}/><b>Choose image or video</b><span>or drop a file here</span></label></div><div className="media-assets"><div className="panel-title"><div><span className="cms-kicker">Your uploads</span><h2>{assets.length} files</h2></div></div>{loading ? <p className="cms-empty">Loading media…</p> : assets.length ? assets.map((asset) => <div className="media-asset" key={asset.id}><span className="media-asset-icon">{asset.contentType.startsWith("video/") ? "▶" : "▧"}</span><div><b>{asset.filename}</b><small>{formatBytes(asset.byteSize)} · {asset.contentType}</small></div><button onClick={() => onUse(asset)}>Add to portfolio</button><button onClick={() => onCopy(asset.url)}>Copy URL</button></div>) : <p className="cms-empty">Your uploaded files will appear here.</p>}</div></section>;
}
